...
when trying to pass in the keyspace name via a ? in a prepared statement lead to this error.
- Caused by: com.datastax.driver.core.exceptions.SyntaxError: Bind variables cannot be used for keyspace names
...
return cassandraTemplate
            .getCqlOperations()
            .queryForList(
                    "SELECT * FROM ?.people_by_first_name WHERE first_name = ?",
                    Person.class,
                    keyspace,
                    firstName);
```
...
return cassandraTemplate
            ...
            .query(
                    "SELECT * FROM ?.people_by_first_name WHERE first_name = ?",
                    new ArgumentPreparedStatementBinder(keyspace, firstName),
...
cant use
```java
cassandraTemplate.select(Query.query(Criteria.where("firstName").is(firstName)), Person.class);
```
due to no way to specify the keyspace

Re-preparing already prepared query is generally an anti-pattern and will likely affect performance. Consider preparing the statement only once. Query='SELECT * FROM myFirstKeyspace.people_by_first_name WHERE first_name = ?'


-------------------------------------------------------------------------
Following on from my last post [Separate keyspaces with Spring Data Cassandra](https://lankydanblog.com/2017/10/22/separate-keyspaces-with-spring-data-cassandra/) we will continue looking into using multiple keyspaces in Cassandra but this time focusing on using a single `CassandraTemplate` to perform queries, rather than creating extra ones for each keyspace that is being used. This removes the need to create extra sessions as each `CassandraTemplate` uses a session to obtain the keyspace is it going to point to. So how do we get the template to actually query the correct keyspace? You quite simply and the name of the keyspace to the query in the same way that you can when writing a normal Cassandra query. Obviously there is slightly more to it than that from our perspective when writing the Java code but that is all the underlying Cassandra query will be doing.
...
I think the best place to start is actually at the end. What I mean is, by looking at what the Cassandra query looks like itself we will have a better understand in what needs to be done to create the correct query.
```sql
...
Know that we have an idea of how the queries will look, lets see how this could be done using Spring Data Cassandra. Below is are two ways of querying different keyspaces.
```java
...
public class PersonRepositoryImpl extends SimpleCassandraKeyspaceRepository<Person, PersonKey>
  ...
  // methods defined in PersonRepository
}
```
...
```
Notice the `CassandraPersistentEntity` that goes into the `MappingCassandraEntityInformation` object (an implementation of `CassandraEntityInformation`). Remember that a `CassandraEntityInformation` object is heavily used inside of the `SimpleCassandraKeyspaceRepository` for retrieving metadata about the entity and it's table. Creating the bean in this way allows you to decide whether to create an instance of `KeyspaceAPersonRepository` or just a simple `PersonRepository`, although the creation of the `CassandraEntityInformation` could be moved `KeyspaceAPersonRepositoryImpl`'s constructor. Again how you create the bean is up to you...
...
I mentioned earlier that there are a few other ways to write queries, below are some examples. I just want to call myself out here, one of these example creates a prepared query every time the query is called. Doing so will output a warning as I am not using prepared queries correctly but thats not the goal of this point so lets just ignore it for now? I'm going to assume your fine with it and carry on...
```java
public class PersonRepositoryImpl extends SimpleCassandraKeyspaceRepository<Person, PersonKey>
    implements PersonRepository {
...
Caused by: org.springframework.data.cassandra.CassandraQuerySyntaxException: Query; CQL [SELECT * FROM ?.people_by_first_name WHERE first_name = ?]; Bind variables cannot be used for keyspace names
```
The last thing I want to mention is why I chose to create a extension of `SimpleCassandraRepository` instead of extending `CassandraTemplate` to allow for choosing the keyspace to query. First a bit of context, because the `@EnableCassandraRepositories` annotation allows for a 'CassandraTemplate` to be specified for it's repositories to use it would make sense to create a template for each keyspace and make keyspace configuration classes with `@EnableCassandraRepositories`. Although this could be done I decided against it because `CassandraTemplate` has method that can take in a `Statement` and CQL (this is how we specified a keyspace earlier) that prevents me from guaranteeing that the keyspace I chose is actually used due to the default keyspace being retrieved from the `Session` bean. 
...
In conclusion the idea of using multiple keyspaces with a single `CassandraTemplate` is not particularly difficult but to provide a solution that can be reused throughout your codebase requires a bit of effort.
