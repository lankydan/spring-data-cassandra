...
*/
public abstract class AbstractKeyspaceCassandraRepository<T, ID> implements CassandraRepository<T, ID> {
  ...
  public List<T> findAll() {
    final Select select = QueryBuilder.select().all().from(keyspace, entityInformation.getTableName().toCql());
    return cassandraTemplate.select(select, entityInformation.getJavaType());
  }

  @Override
  public List<T> findAllById(final Iterable<ID> ids) {
    final List<ID> idCollection = Streamable.of(ids).stream().collect(StreamUtils.toUnmodifiableList());
    return cassandraTemplate.select(QueryBuilder.select().from(keyspace, entityInformation.getTableName().toCql()).where(in(entityInformation.getIdAttribute(), idCollection)), entityInformation.getJavaType());
//    return cassandraTemplate.select(Query.query(where(entityInformation.getIdAttribute()).in(idCollection)),
//            entityInformation.getJavaType());
  ...
  public boolean existsById(ID id) {
    ...
    final CassandraPersistentEntity<?> entity = converter.getMappingContext().getRequiredPersistentEntity(entityInformation.getJavaType());
  ...
  public long count() {
    final CassandraConverter converter = cassandraTemplate.getConverter();
    final Select select = QueryBuilder.select().countAll()
            .from(keyspace, converter.getMappingContext().getRequiredPersistentEntity(entityInformation.getJavaType()).getTableName().toCql());
//    final Long count = cassandraTemplate.getCqlOperations().queryForObject(select, Long.class);
...
//    getConverter().write(id, delete.where(), entity);
  ...
  public void deleteAll() {
    cassandraTemplate.getCqlOperations().execute(QueryBuilder.truncate(keyspace, getPersistentEntity().getTableName().toCql()));
  }
...
//    }
  ...
  public Optional<T> findById(ID id) {
    final CassandraPersistentEntity<?> persistentEntity = getConverter().getMappingContext()
